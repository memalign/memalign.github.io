...
document.getElementById('custom_colors_box').addEventListener('click', function (event) {
	...
	if (event.target.className == 'custom_colors_preview') {
		...
		colorPickerComp.setColorWithString(color);
	}
});
...
colorPickerComp.addCustomColor = function () {
	// Getting the color
	const color = `rgba(${colorPickerComp.r}, ${colorPickerComp.g}, ${colorPickerComp.b}, ${colorPickerComp.alpha})`;
...
let displayContextMenu = function(event) {
	// Making sure the users has selected a color preview
	if (event.target.className == 'custom_colors_preview') {
		// Preventing default
...
document.getElementById('custom_colors_box').addEventListener('long-press', function (event) {
  displayContextMenu(event)
});
