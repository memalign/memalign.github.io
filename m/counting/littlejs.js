...
{
    if (enableAsserts && !assert) {
        output ? console.log(assert, output) : console.log(assert);
        MAUtils.logStackTrace()
        MAUtils.throwError("assertion failed. " + output)
    }
}
...
class EngineObject
{
    ...
    ma_containsPoint(p) {
      ...
      const bottomLeft = this.pos.subtract(halfSize)
      const topRight = this.pos.add(halfSize)
      return (bottomLeft.x <= p.x) &&
        (topRight.x >= p.x) &&
        (bottomLeft.y <= p.y) &&
        (topRight.y >= p.y)
    }
...
function inputInit()
{
    ...
    onmousedown   = (e)=>
    {
        ...
        // memalign: I added this line so ma_handleMouseDown would work
        mousePos = screenToWorld(mousePosScreen);
...
// Give objects a direct call from a user input event
// Some APIs (e.g. SpeechSynthesis) only work when directly invoked as
// a result of user input.
function ma_handleMouseDown() {
  // Copy engineObjects before sorting to avoid changing engine behavior
  // Use the same sort that render uses then reverse it so we break renderOrder ties the same way
  const engineObjectsFrontToBack = [...engineObjects].sort((a,b )=> a.renderOrder - b.renderOrder).reverse()

  for (const o of engineObjectsFrontToBack) {
    // call into top level objects
    if (!o.parent) {
      o.ma_handleMouseDown()
    }
  }
}

function ma_handleTouchEnd() {
  // Copy engineObjects before sorting to avoid changing engine behavior
  // Use the same sort that render uses then reverse it so we break renderOrder ties the same way
  const engineObjectsFrontToBack = [...engineObjects].sort((a,b )=> a.renderOrder - b.renderOrder).reverse()
...
function touchInputInit()
{
    ...
    // memalign: I commented this out so both mouse and touch work at the same time
    ...
    function handleTouchDefault(e)
    {
        ...
        if (touching)
        {
            ...
            // memalign: I added this line so ma_handleTouch would work
            mousePos = screenToWorld(mousePosScreen);
...
function engineInit(gameInit, gameUpdate, gameUpdatePost, gameRender, gameRenderPost, imageSources=[], rootElement=document.body)
{
    ...
    function updateCanvas()
    {
        ...
        {
            // memalign: I copied this logic from tap toy slice so the canvas can fill the whole screen
    ...
    // setup html
    // memalign: I commented this out to replace it with a normal style sheet
  /*
    const styleRoot =
