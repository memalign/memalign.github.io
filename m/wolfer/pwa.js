...
let PWAIconURL = './wolfer-1110.png';
...
if (gameName) {
  manifest.name = GameGenerators[gameName].name;
  manifest.short_name = GameGenerators[gameName].shortName;
}

if (typeof classicMode !== "undefined" && classicMode) {
  PWAIconURL = './classic-munchers-1668.png';

  manifest.name = manifest.name.replace("Wolfer", "Muncher");
  manifest.short_name = manifest.short_name.replace("Wolfer", "Muncher");
}

if (gameName) {
  PWAIconURL = GameGeneratorIconURLForGameName(gameName, 2);
}
