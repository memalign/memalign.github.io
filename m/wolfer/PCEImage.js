...
class PCEImage {
  ...
  // Returns base64 data URL, with optional text drawn on it
  generatePNGWithText(scale, str, xOffset, yOffset, fontSize) {
    const cacheKey = this._imageStr + "_" + scale + "_" + str + "_" + xOffset + "_" + yOffset + "_" + fontSize
    const cachedImg = sPCEImageCache[cacheKey]
  ...
  newPCEImageByMirroring() {
    let imageStr = ""
    let comma = ""
    for (let y = 0; y < this.height; ++y) {
      let line = this.imageStrLineAtPixelRow(y)
      let retLine = line.split("").reverse().join("")
      imageStr += comma + retLine
      comma = "\n"
    }
  ...
  // pixelTransformFn(x, y, pixelChar) => newPixelChar
  newPCEImageFromPixelTransform(pixelTransformFn) {
    let newImageStrLines = []
    let comma = ""
    for (let y = 0; y < this.height; ++y) {
      let line = this.imageStrLineAtPixelRow(y)
      let newLine = ""
      for (let x = 0; x < this.width; ++x) {
        let pixelChar = line.charAt(x)
        let newPixelChar = pixelTransformFn(x, y, pixelChar)
        newLine += newPixelChar
      }
      newImageStrLines.push(newLine)
    }

    let colorLines = this.imageStrLines.slice(0, this.firstPixelLineIndex)

    let imageStr = colorLines.join("\n") + "\n" + newImageStrLines.join("\n")
